    ...
    <div class="mx-3">
      <v-form ref="registerForm" v-model="isRegisterValid" @submit.prevent fast-fail>
        <XyzTransition appear-visible xyz="fade left-5">
          <v-card max-width="640" class="rounded-lg pa-2 mx-auto mb-6" variant="elevated" elevation="6">
            <v-card-subtitle class="my-4">Isi data & password untuk akun anda</v-card-subtitle>
            <v-card-text>
              <v-row>
                <v-col cols="12" sm="6">
                  <v-text-field label="Nama depan" flat v-model="formData.first_name" :rules="[required]"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field label="Nama belakang" flat v-model="formData.last_name"></v-text-field>
                </v-col>
                <v-col cols="12">
                  <v-text-field label="No telepon" flat v-model="formData.phone" @input="filterTel"></v-text-field>
                </v-col>
                <v-col cols="12">
                ...
                <v-col cols="12">
                  <v-text-field label="Konfirmasi password" flat v-model="formData.password_confirmation" type="password"
                    hint="Minimal 8 karakter"
                    :rules="[required, min(formData.password, 8), passwordEqual(formData.password_confirmation, formData.password_confirmation)]"></v-text-field>
                </v-col>
              </v-row>
            </v-card-text>
            <v-card-actions>
